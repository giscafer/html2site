...
export default function renderNav(
  ...
  return `<ul class="menu-item">
${groupedFiles
  .filter((f: any) => f.name !== 'template')
  .map((f) => {
    // console.log(f);
    if (f.type === 'dir') {
...
function renderActive(text: string, href: string, active: boolean) {
  return `<a class="filename ${
    active ? 'active' : ''
  }" href="javascript:void(0)" data-filename="${
    href === 'template.html' ? 'readme.html' : href
  }">${text === 'template' ? 'readme' : text}</a>`;
}

function getIndexFile(files: FileTree<IndexFile>): File<IndexFile> | undefined {
  return files.find(
    (e) => e.type === 'file' && e.value.text === 'index'
  ) as File<IndexFile>; // Stupid TS
}
